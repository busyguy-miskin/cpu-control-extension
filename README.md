...
- **第 2 层(ryzenadj 调优)对死机几乎没有帮助。** 收紧温度墙/功耗并不能有效阻止 MCE 死机,该层的实际价值比预期低得多。
- **第 1 层(关闭睿频 boost 锁)效果显著,基本消灭死机。** 但 2.9GHz 锁频带来的性能损失在长期使用中难以接受,最终放弃了常开 boost 锁的方案。
- **(本机个案)死机报错锁定在 CPU 2/3 两个线程(同一个物理核心)上。** 这只是我这台机器的具体情况,不代表普遍规律——其他机器的故障核心位置可能不同。由于本机故障高度集中,我决定改用**屏蔽该核心**(内核 `nohz_full`/`isolcpus`,或直接 `maxcpus`/核心禁用)进行测试,验证屏蔽问题核心能否在保留睿频的同时消除死机。

因此,对我而言当前的双层方案更多是记录性质的存档;后续验证方向是核心屏蔽,而非继续调参。
