...
- **死机报错目前锁定在 CPU 2/3 两个线程(同一个物理核心)上。** 由于故障高度集中,决定改用**屏蔽该核心**(内核 `nohz_full`/`isolcpus`,或直接 `maxcpus`/核心禁用)进行测试,验证屏蔽问题核心能否在保留睿频的同时消除死机。

因此,当前的双层方案更多是记录性质的存档;后续验证方向是核心屏蔽,而非继续调参。
